        ...
        <div class="product-to-be-upgraded">
            <div class="row">
                ...
                <div class="col-sm-3 text-right">
                    <a href="{$WEB_ROOT}/clientarea.php?action=productdetails&id={if $serviceToBeUpgraded->isService()}{$serviceToBeUpgraded->id}{elseif $serviceToBeUpgraded->isAddon()}{$serviceToBeUpgraded->service->id}{/if}"
                ...
                <div class="column col-sm-{if count($upgradeProducts) >= 3}4{else}6{/if}">
                    <div class="product">
                        ...
                        <div class="footer">
                                ...
                                {if $allowMultipleQuantities}
                                    <div class="text-right margin-bottom-5">
                                        <label class="checkbox-inline">
                                            ...
                                            <input type="number" name="qty" min="{$minimumQuantity}" value="{$currentQuantity}"
                                ...
                                <select name="billingcycle" class="form-control">
                                    {foreach $product->pricing()->allAvailableCycles() as $cycle}
                                        {if $permittedBillingCycles->showCycleForProduct($cycle->cycle())}
                                            <option value="{$cycle->cycle()}" {if $permittedBillingCycles->isCycleDisabledForProduct(
                                                                            $product->id,
                                                                            $cycle->cycle()
                                                                )} disabled {/if}>
                            ...
                            <button type="submit" class="btn btn-block" id="btnUpgradeSelect-{$product->productKey}"
