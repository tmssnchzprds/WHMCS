    ...
    <form method="post" action="{$smarty.server.PHP_SELF}?step=3" enctype="multipart/form-data" role="form">
                ...
                <div class="form-group col-sm-5">
                    ...
                    <select name="relatedservice" id="inputRelatedService" class="form-control">
                        ...
                        {foreach from=$relatedservices item=relatedservice}
                            <option value="{$relatedservice.id}" {if $relatedservice.id eq $selectedservice} selected="selected"
                                {/if}>
        ...
        <p class="text-center">
            <input type="submit" id="openTicketSubmit" value="{$LANG.supportticketsticketsubmit}"
                class="wdes-submit disable-on-click{$captcha->getButtonClass($captchaForm)}" />
