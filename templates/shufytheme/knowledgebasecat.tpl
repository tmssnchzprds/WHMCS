{if $kbarticles}
<div class="knowledgebase__search__bar__wrapper mb-7">
	<form role="form" method="post" action="{routePath('knowledgebase-search')}">
		<input type="text"  id="inputKnowledgebaseSearch" name="search" class="form-control" placeholder="{lang key='clientHomeSearchKb'}" value="{$searchterm}" />
...
{if $kbarticles || !$kbcats}
	<div class="kbcat__wrapper mb-7">
		<div class="tab__header mb-4">
			{if $tag}
				<h3 class="coodiv-text-9 mb-0">{lang key='kbsuggestions'} '{$tag}'</h3>
			{else}
				<h3 class="coodiv-text-9 mb-0">{lang key='knowledgebasearticles'}</h3>
			{/if}
		...
		<div class="shadow__card kb__category__list kbmostviews__list overflow-hidden">
			...
			<div class="card__footer d-flex align-items-center justify-content-between">
				...
				{if $kbcurrentcat.editLink}
					<a href="{$kbcurrentcat.editLink}" class="btn btn-default btn-sm">
						{lang key='edit'}
					</a>
				{/if}
			</div>
