						...
						<div class="field-container">
							...
							<div class="default__shadow__panel__body">
								...
								$(document).ready(function() {
									...
									let randomHostname = 'server-' + Math.floor(Math.random() * 10000);
									$('#inputHostname').val(randomHostname);

									// Generate random password with 12 characters including special characters
									let characters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*()';
									let randomPassword = Array.from({ length: 12 }, () => characters.charAt(Math.floor(Math.random() * characters.length))).join('');
									$('#inputRootpw').val(randomPassword);
								});
