    ...
    <table id="tableEmailsList" class="table table-list hidden">
        ...
        <tbody>
            ...
            <tr onclick="popupWindow('viewemail.php?id={$email.id}', 'emailWin', '650', '450')">
                ...
                <td class="text-center"><input type="button" class="btn btn-small btn-prussian-medio" value="{$LANG.emailviewmessage}" onclick="popupWindow('viewemail.php?id={$email.id}', 'emailWin', '650', '450', 'scrollbars=1,')" /></td>
            </tr>
            {/foreach}
        </tbody>
    </table>
